...
class Staff {
  constructor(canvas, x, y, startOctave, lineGap = 10, lineLength = 500) {
    ...
    this.soundPlayer = new SoundPlayer(this.notes);
  ...
  drawNote(note, accidental = '', isChordNote = false) {
    ...
    const notes = isChordNote ? [this.notes[this.notes.length - 1].note, noteName] : [noteName];
    this.soundPlayer.playNotes(notes);
